...
func (s *Server) handleSingleMessage(ctx context.Context, raw []byte) ([]byte, error) {
	...
	default:
		...
		return marshalResponse(responseEnvelope{
			JSONRPC: jsonRPCVersion,
			ID:      req.ID,
			Error:   newRPCError(rpcCodeMethodFound, "method not found", map[string]any{"method": req.Method}),
		})
	}
}
...
func (s *Server) handleToolCall(ctx context.Context, req requestEnvelope) ([]byte, error) {
	...
	if !ok {
		return marshalResponse(responseEnvelope{
			JSONRPC: jsonRPCVersion,
			ID:      req.ID,
			Error:   newRPCError(rpcCodeMethodFound, "tool not found", map[string]any{"tool": params.Name}),
		})
	}
